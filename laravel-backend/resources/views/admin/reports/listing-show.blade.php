...
@section('title', 'Listing Report #' . $report->id)
...
<style>
    ...
    .detail-card .card-header i { margin-right: .4rem; color: var(--blue); }
    ...
    .evidence-box i { font-size: 1.5rem; color: var(--blue); }
    ...
    .listing-preview .listing-img-placeholder {
        width: 80px; height: 80px; border-radius: .375rem;
        background: var(--blue-lt); display: flex; align-items: center;
        justify-content: center; flex-shrink: 0;
    }
    .listing-preview .listing-img-placeholder i { font-size: 1.8rem; color: var(--blue); }
    .listing-meta .listing-title { font-weight: 700; margin-bottom: .25rem; }
    .listing-meta .listing-price { color: var(--green); font-weight: 600; }
    .listing-meta small { color: var(--gray); }

    /* ── Reporter card ─────────────────────────────────────────── */
    .reporter-info { display: flex; align-items: center; gap: .9rem; }
    .reporter-avatar {
    ...
    .reporter-avatar img { width: 44px; height: 44px; border-radius: 50%; object-fit: cover; }
    .reporter-name { font-weight: 600; margin-bottom: .1rem; }
    .reporter-sub { font-size: .8rem; color: var(--gray); }
    ...
    .btn-action.btn-disable  { background: #fff3e0; color: #e65100; border: 1px solid #ffcc80; }
    ...
    .status-form textarea {
        font-size: .85rem; resize: vertical; min-height: 80px;
    }
    ...
    .back-link:hover { text-decoration: underline; color: var(--navy); }
</style>
@endpush
...
<div class="report-header">
    <span class="badge-type"><i class="fas fa-flag mr-1"></i>Listing Report</span>
...
<div class="row">
    ...
    <div class="col-lg-8">
        ...
        {{-- Reported Listing --}}
        <div class="detail-card">
            <div class="card-header"><i class="fas fa-store"></i>Reported Listing</div>
            <div class="card-body">
                @if($report->listing)
                <div class="listing-preview">
                    @php $primaryImage = $report->listing->listingImages->where('is_primary', true)->first() ?? $report->listing->listingImages->first(); @endphp
                    @if($primaryImage)
                        <img src="{{ Storage::url($primaryImage->image_path) }}" alt="Listing image">
                    @else
                        <div class="listing-img-placeholder"><i class="fas fa-image"></i></div>
                    @endif
                    <div class="listing-meta">
                        <div class="listing-title">{{ $report->listing->title }}</div>
                        <div class="listing-price">₱{{ number_format($report->listing->price, 2) }}</div>
                        <small>
                            <span class="status-badge status-{{ $report->listing->listing_status }} mr-1" style="font-size:.68rem; padding:.15rem .5rem;">
                                <span class="dot"></span>
                                {{ ucfirst(str_replace('_', ' ', $report->listing->listing_status)) }}
                            </span>
                            &bull; {{ $report->listing->campus_location }}
                        </small>
                        <div class="mt-1">
                            <small class="text-muted">
                                Seller:
                                <strong>{{ optional($report->listing->user)->fullName() ?? 'Unknown' }}</strong>
                                &bull; Listed {{ $report->listing->created_at->diffForHumans() }}
                            </small>
                        </div>
                    </div>
                </div>
                @else
                    <p class="text-muted mb-0"><i class="fas fa-exclamation-triangle mr-1 text-warning"></i>Listing has been deleted.</p>
                @endif
            </div>
        </div>
        ...
        <div class="detail-card">
            ...
            <div class="card-body">
                ...
                <div class="reporter-info">
                    ...
                    <div>
                        <div class="reporter-name">{{ $report->reporter->fullName() }}</div>
                        <div class="reporter-sub">{{ $report->reporter->email ?? $report->reporter->student_id }}</div>
                        <div class="reporter-sub">
                            <span class="status-badge status-{{ $report->reporter->apiStatus() }}" style="font-size:.68rem; padding:.15rem .5rem;">
                                <span class="dot"></span>
        ...
        <div class="action-panel">
            ...
            <div class="ap-body">
                ...
                <div id="form-review" class="status-form">
                    <form method="POST" action="{{ route('admin.reports.listings.status', $report->id) }}">
                        @csrf @method('PATCH')
                ...
                {{-- Disable Listing --}}
                @if($report->listing && $report->listing->listing_status !== 'suspended')
                <button class="btn-action btn-disable {{ $resolved ? 'btn-disabled-state' : '' }}"
                    onclick="{{ $resolved ? 'void(0)' : 'toggleForm(\'form-disable\')' }}">
                    <i class="fas fa-ban"></i> Disable Listing
                    @if($resolved) <small class="ml-auto text-muted">Report closed</small> @endif
                </button>
                <div id="form-disable" class="status-form">
                    <form method="POST" action="{{ route('admin.reports.listings.disable', $report->id) }}">
                        @csrf
                        <div class="alert alert-warning py-2 px-3" style="font-size:.82rem;">
                            <i class="fas fa-exclamation-triangle mr-1"></i>
                            This will suspend the listing and auto-resolve this report.
                        </div>
                        <div class="form-group">
                            <label class="form-label">Admin Note <span class="text-muted font-weight-normal">(optional)</span></label>
                            <textarea name="admin_note" class="form-control" placeholder="Reason for disabling..."></textarea>
                        </div>
                        <button type="submit" class="btn btn-sm btn-warning btn-block text-white">
                            <i class="fas fa-ban mr-1"></i> Confirm Disable
                        </button>
                    </form>
                </div>
                @elseif($report->listing && $report->listing->listing_status === 'suspended')
                <button class="btn-action btn-disable btn-disabled-state" disabled>
                    <i class="fas fa-ban"></i> Listing Already Suspended
                </button>
                @endif
                ...
                <table class="table table-sm table-borderless mb-0" style="font-size:.85rem;">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right pr-3">Listing Report</td>
                        </tr>
                        <tr>
